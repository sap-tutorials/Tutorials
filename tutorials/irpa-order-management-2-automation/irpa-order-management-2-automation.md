...
## Details
### You will learn
  - Create an automation in Cloud Studio
  - Use Excel activities in an automation

### The user story

In this automation, you will do the following:

1. Open an Excel workbook.

2. Set the columns for Order Number and Status in the workbook.

3. From the [Browse Orders application](https://openui5.hana.ondemand.com/test-resources/sap/m/demokit/orderbrowser/webapp/test/mockServer.html), get the order number and order status.
...
1. Check the dependencies by clicking **Menu** &nbsp; !![Menu](menu.png) in the Project Explorer.

    >Alternatively, you can click **manage the project properties** icon ![manage project properties icon](manage-project-properties.png).

2.  Select **Dependencies** from the dropdown list.

    !![select-dependencies](select-dependencies.png)

3.  Click **Manage Dependencies** icon.

    !![manage-dependencies](manage-dependencies.png).

4.  From the **Project Properties** popup screen, click **Dependencies** to manage your project dependencies.
...
1.  Click !![Plus](plus.png), then hover over **Create** and select **Automation** from the artifact dropdown.

    !![choose-automation](choose-automation.png)

2.  From the **Configure agent version** popup, select the **Desktop Agent version** installed on your machine and click **Confirm**.

    !![configure-desktop-agent](configure-desktop-agent.png)

3. In the **Create Automation** popup, input the following information:
...
2.  Click on **Browse Orders** and click **Define Screen Activities** to view the activities related to **screen**.
...
The automations looks as below
...
1.  Click **Browse Orders** in the workflow and from the **Parameters** panel, click **Define Screen Activities**.
...
7.  Select and edit the activity **Create Number Variable**.
    ...
    -	Click **Open the Expression editor** to open the **Expression Editor**.  

        !![expression editor icon](expression-editor-icon1.png).

    -	In the **Expression Editor**, input this text:
    ...
    -	Rename the **Output Parameters** as `myExcelRow`. The step will now look like this:

        !![create-number-variable-final](create-number-variable-final.png)

8.  Drag-and-drop **Set Values (Cells)** activity in the **For** loop below Create Number Variable activity.
...
9.  Repeat Steps 4-8 for the element **Order Status** using the below information:
...
Your complete automation should look like this:

!![complete-automation](complete-automation.png)

You have completed building the automation.
...
To test the automation, click !![test icon](Test.png).
