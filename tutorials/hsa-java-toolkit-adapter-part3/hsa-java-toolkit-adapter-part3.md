...
[ACCORDION-BEGIN [Step 5: ](Create an Adapter Configuration File(.xml))]
...
**a.** Start by creating an `<Adapter>` element and include all the elements from the steps below within it.
...
```
  ...
  - **vi.** Specify parameter elements and values for them. Don't forget to replace `<your-ip-address>` with your client's `ip address`.
```html
...
**c.** Create a `<URI>` element and specify the `URI` used to connect to the streaming project. The address of the streaming server should be followed by `/workspace/project-name`. Don't forget to replace `<streaming-server-address>` and `<port>` with your server's `ip address` and `port number`.
```html
