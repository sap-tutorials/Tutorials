...
## Prerequisites
 ...
 - You have subscribed to the SAP Build Work Zone, advanced edition application in your subaccount. For more information, see [Initial Setup](https://help.sap.com/docs/build-work-zone-advanced-edition/sap-build-work-zone-advanced-edition/initial-setup-and-concepts)
 - You have subscribed to the Custom Domain Manager in the same SAP BTP subaccount in which SAP Build Work Zone, advanced edition application is subscribed.
...
2. Click **Create** to open a wizard to create each required configuration.
    ...
    ![Create configuration](10a-create-configuration.png)
...
This step is done in the Custom Domain Manager. You'll create a new server certificate for custom domains associated with the SAP Build Work Zone, advanced edition application and runtime destinations. You will also create a certificate for your custom domains from a trusted certificate authority (CA). Make sure that the Cloud Foundry environment is enabled for your subaccount.

1. Choose the **Server Identities** tile.

     ![Server certificates tile](14-server-identities-tile.png)

2. Choose **Create**.

    ![Choose create](14a-choose-create.png)
...
3. The wizard opens with the **Select Name** step in focus. Now do the following:
    ...
    - Enter the desired name, and then click **Next Step**.
        ...
        ![Select name](15-select-name.png)
    ...
    - In the **Select Target** step, keep the landscape with the default value, which should be the main landscape (e.g. `cf-eu10`). Then click **Next Step**.
        ...
        ![Selecttarget](16-select-target.png)
    ...
    - In the **Certificate Subject** step, keep the **CommonName (CN)** parameter with the default value, then click **Finish**. 
    ...
    - In the **Key Configuration** step, keep the default key type.
        ...
        ![Keyconfiguration](18a-key-configuration.png)

    - In the **Summary** step, click the **Finish** button.

        ![Summary](18b-summary-step.png)
...
4. Select the server certificate that you created to expand the details section. To order and install your new server certificate, you must first create the certificate signing request and then send this file to a trusted certificate authority of your choice to get it signed.
    ...
    ![Server certificates details](19b-server-certificate-details.png)
...
3. Change the URL to: `https://<Subaccount subdomain>.dt.workzone.cfapps.<Data Center>.hana.ondemand.com/sites#SubAccount-Settings` to access the subaccount settings.
...
**Prerequisite**: - Configure the identity Authentication service and the Identity Provisioning service. For more information, see Option 1 Step 3 in: [Run the Configurator](https://help.sap.com/docs/build-work-zone-advanced-edition/sap-build-work-zone-advanced-edition/run-configurator)
...
After obtaining the custom domain, you must create a server certificate and re-run the onboarding Configurator and use your custom domain instead of the default domain. Using a non-custom domain such as `ondemand.com` is not supported once you've already configured a custom domain.
...
3. Once the certificate is signed by a CA, make sure you have a .crt file that includes these 3 certificates in the following order: a certificate, an intermediate certificate, and a root certificate.
...
1. Before running the Configurator, make sure you have the OAuth client details ready. You will have to enter them in the Configurator. In the **Administration Console**, go to **External Integrations** -->  **OAuth Clients**, and copy the details of the OAuth client that was created when you ran the Configurator for the first time. 
    ...
    ![Add OAuth Client](29-oauth-client.png)
...
2. Reset the Configurator to make sure that all previously entered data has been removed. To do this, follow these steps:
    ...
    - You are now in the Configurator wizard. The screen you need is not within the Configurator wizard. So you need to exit the wizard by using the back icon (<).
        ...
        ![Configurator icon back button](29a-Configurator-icon.png) 

    - Go back until you get to the screen where you can select the configuration option.  

        ![Configurator option](30-configurator-option.png) 
...
5. Open the SAP Build Work Zone, advanced edition Configurator. In the "Create a Destination to SAP SuccessFactors (Optional)" step that opens, click **Skip Step**, unless you are using SAP SuccessFactors. 
     ...
     ![Open Configurator](33-open-configurator.png)
...
9. In the **Private Key** field, upload the key from step 3 above.   
    ...
    ![Enter certificates](34-certificates.png)
