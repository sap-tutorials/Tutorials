...
SAP HANA Cloud provides the ability to store and perform queries on [spatial data](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-spatial-reference/sap-hana-cloud-sap-hana-database-spatial-reference) such as a point, a line segment, or a polygon.
...
2. Under the Advanced Settings Tab, ensure the Triple Store option is enabled.
    ...
    *The knowledge graph feature is not available for free tier instances. To view what is available in the free tier please refer to [SAP HANA Database License](https://help.sap.com/docs/hana-cloud/sap-hana-cloud-administration-guide/sap-hana-database-license?locale=en-US)*

    To learn more about knowledge graphs see [Connecting the Facts: SAP HANA Cloud’s Knowledge Graph Engine for Business Context](https://community.sap.com/t5/technology-blogs-by-sap/connecting-the-facts-sap-hana-cloud-s-knowledge-graph-engine-for-business/ba-p/13888597).
...
4. To visualize and examine the knowledge graph database objects, you must first enable the feature in your preferences.  Click on your account
    ...
    > ![Select Objects](select-objects.png)
...
5. Run a query to find hotels that have an indoor pool.
...
The following steps will create a property graph workspace that can display the distance between hotels in a state.
 For details about the differences between a knowledge graph and a property graph see [Choosing Between Knowledge Graphs and Property Graphs in SAP HANA Cloud and Why Both Matter](https://community.sap.com/t5/technology-blogs-by-sap/choosing-between-knowledge-graphs-and-property-graphs-in-sap-hana-cloud-and/ba-p/14074575).
...
1. Create a vertex table that represents distances between hotels by executing the following in the SQL console. Run the SQL in the HOTELS schema as USER1. 
...
4. Navigate to database objects, select the **Graph Workspaces tab**, and open the previously created graph workspace to view its properties.

    ![DISTANCEGRAPH properties](distance-graph-workspace.png)

    > Ensure Graph Workspaces is enabled in the database objects settings.
...
2. Set the vertex name in **Graph Viewer** by clicking on the settings icon. 

    Set the vertex label to `NAME`.

    ![DISTANCEGRAPH vertex labels](vertex-label.png)

3. Apply a filter to vertices where `STATE` is NY.  After specifying the filter, apply it by pressing the Apply button.
...
4. Highlight the Long Island vertex using the property graph viewer settings. You may do so by selecting a color.

    ![DISTANCEGRAPH highlighted Long Island](long-island.png)

5. View the details by clicking on the vertex or edge of interest. 

    ![View Details](view-details.png)
...
2. After removing the previously applied filters, navigate to the property graph viewer, and select the algorithms tab. Update the Algorithm field to "Shortest Path", specify the values shown below, and click Apply.
    ...
    After applying the algorithm, the shortest path is highlighted in the graph viewer, showing the optimal route between the two specified hotels as a chain of connected vertices and edges.
...
>The creation of a JSON collection is not supported in the SAP HANA Cloud free tier.  To view what is available in the free tier please refer to [SAP HANA Database License](https://help.sap.com/docs/hana-cloud/sap-hana-cloud-administration-guide/sap-hana-database-license?locale=en-US).
...
1. Enable the JSON document store.  
    ...
    ![JSON Document Store](enable-document-store.png)
...
6. The properties of the JSON collection can also be viewed under **JSON Collections** in Database Objects.  

    ![Collection properties](collection-properties.png)

    Click on the **JSON Data tab** to view the JSON text.
...
1. Find a JSON collection to be imported.  The example below uses the Parks dataset from [City of Waterloo Open Data](https://opendata-city-of-waterloo.opendata.arcgis.com/) downloaded as a GeoJSON file. Rename the file to be `ParksinWaterloo.geojson` to match the code below. 
...
5. Query the imported collection.
   ...
   SELECT "f"."properties"."PARK_NAME", "f"."properties"."TENNIS_CRT" FROM "HOTELS"."WATERLOO_PARKS" UNNEST "features" as "f" ORDER BY "f"."properties"."TENNIS_COURT" DESC;
   ```
...
3. Unzip the downloaded `shapefile`. Upload all of the extracted files (`Points_of_Interest.shp`, `.dbf`, `.prj`, `.shx`, `.cpg`, `.xml`) to your data lake Files instance using by navigating to the Data Lake Files app, selecting the desired file, and using the upload button.
    ...
    ![Data Lake Files Upload](data-lake-files-upload.png)

4. If not already done, set up the trust between your SAP HANA database and data lake Files and create the HTTPS PSE and credential. The full setup is covered in the [Export and Import Data with SAP HANA Cloud Data Lake Files](hana-cloud-sqltools-import-export) tutorial.
    ...
    Import the shapefile using SQL, replacing the `hdlfs://` path with your data lake Files REST API endpoint.

   ```SQL
   IMPORT HOTELS.POI_WATERLOO AS SHAPEFILE FROM 'hdlfs://<your-data-lake-files-endpoint>/Points_of_Interest' WITH SRID 4326 CREDENTIAL 'DL_FILES';
   ```

    >The REST API endpoint can be copied by clicking the three dots in the **Actions** column next to your data lake Files instance in SAP HANA Cloud Central. Replace `https://` with `hdlfs://` and append the base filename without extension. For additional details see [Import ESRI Shapefiles Using SQL Commands](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-spatial-reference/import-esri-shapefiles-using-sql-commands) and [Spatial Reference Systems](https://help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-spatial-reference/spatial-reference-systems-srs-and-spatial-reference-identifiers-srid).

    Verify the import was successful.

   ```SQL
   SELECT * FROM HOTELS.POI_WATERLOO;
   ```

    ![view import](view-import.png)

5. View the table in **Database Objects**.  Notice that the points of interest locations are stored in a column of type `ST_GEOMETRY`.

    ![view table editor](view-table-metadata.png)

6. Perform the below query.

   ```SQL
   SELECT FACILITY, SHAPE.ST_AsWKT(), SHAPE FROM HOTELS.POI_WATERLOO;
   ```
    ...
    >For an optional example of importing a GeoJSON file as a JSON collection, see [Export and Import Data and Schema with SAP HANA Database Explorer](hana-dbx-export-import).

### Use spatial functions in a query

1. The following statement shows the list of points of interest within 3 kilometers of the `Delta` hotel. Ensure you are in the `HOTELS` schema.
   ...
   ```
    ...
    ![Spatial Function Result](spatial-functions-result.png)
