...
![Schematic overview of CocoaPods resolving a dependency](cocoapods-1-overview.png)
...
###### 2.1.5.1 Device
Connect your iPhone to your Mac. Then, in Xcode, in the top menu, navigate to "Window" > "Devices and Simulators". Add your connected device via the "+" button to the bottom left as shown in Figure 4.

###### 2.1.5.2 Development Certificate
In the left-hand side menu, in the "Certificates" section, select "Development" to see your list of certificates, as shown in Figure 5. Hit the "+" button to add a new one, selecting "iOS App Development" as the purpose of your certificate (Figure 6). You will then need to create a Certificate Signing Request (CSR) on your Mac and upload it to the Developer page to receive your certificate. Finally, download and install the certificate to your local Keychain.
...
Once you created both your distribution and development certificates, make sure to download them and to add them to the Keychains of both your build and development node, as shown in Figure 7.

###### 2.1.5.4 App ID
In the left-hand side menu, in the "Identifiers" section, select "App IDs" to see your list of App IDs, as shown in Figure 8. Create a new one to your liking, but remember to always use that specific identifier in the remainder of this tutorial.

###### 2.1.5.5 Development Provisioning Profile
In the left-hand side menu, in the "Provisioning Profiles" section, select "Development" to see your list of development provisioning profiles, as shown in Figure 9. Hit the "+" button to add a new one, selecting "iOS App Development" as the purpose. When asked, make sure to select the App ID, device and development certificate you created. In the end, give your provisioning profile an arbitrary name and save it.
...
![Figure 4: Adding a device to your Developer account](xcode-1-device.png)

*Figure 4: Adding a device to your Developer account*

![Figure 5: Development certificate list](apple-2-devcert-add.png)

*Figure 5: Development certificate list*

![Figure 6: Creating a new development certificate](apple-3-devcert-create.png)

*Figure 6: Creating a new development certificate*

![Figure 7: iOS development and distribution certificates in the Keychain](apple-4-keychain.png)

*Figure 7: iOS development and distribution certificates in the Keychain*

![Figure 8: Registering a new application ID with Apple](apple-1-appid.png)

*Figure 8: Registering a new application ID with Apple*

![Figure 9: Creating a new provisioning profile](apple-5-provprof.png)

*Figure 9: Creating a new provisioning profile*
...
```
You can now just copy the displayed password and paste it into the Jenkins UI. When asked which plugins to install, just go with the default selection, as shown in Figure 4. Among other things, the defaults enable us to leverage Pipelines, a great feature that enables us to also version control build scripts. Eventually you will be asked to optionally create a new admin user (Figure 11).
...
which will print your current PATH variable. Copy the value and in Jenkins, paste it in the form prefix by "$PATH:" as shown in Figure 12. Save the settings and your Jenkins server is ready to receive iOS builds.

![Figure 10: Installing Jenkins features](jenkins-1-getting-started.png)

*Figure 10: Installing Jenkins features*

![Figure 11: Creating an admin user in Jenkins](jenkins-2-create-admin.png)

*Figure 11: Creating an admin user in Jenkins*

![Figure 12: Setting the PATH variable in Jenkins](jenkins-3-path.png)

*Figure 12: Setting the PATH variable in Jenkins*
...
Once the server has started, navigate to [http://localhost:8081](http://localhost:8081) to open the Artifactory web interface. When it is asking you which repositories to create, pick only CocoaPods or everything Mobile-related as shown in Figure 13. The absolute minimum you should select, however, is CocoaPods.

When everything is set up, Artifactory will take you to the main dashboard, containing a neat "Set Me Up" section that nicely explains how to proceed with your system setup, as shown in Figure 14. Select the ``cocoapods-local`` entry to learn how to configure your Mac for this repository. In a nutshell, you need to do three things: Installing the CocoaPods Artifactory extension, adding the ``cocoapods-local`` repository to your Mac, and configuring credentials to access that repository. To do so, run
...
Now the iOS SDK is available to CocoaPods clients, and your local tools can find them. You can take a look at your ``cocoapods-local`` repository as shown in Figure 18 to make sure everything is in order. While doing so, also take note of the version displayed (in this case ``2.1.400``), which you need to match in your dependency specification later on.

![Figure 17: Exporting SAP Cloud Platform SDK for iOS frameworks from the Assistant](assistant-1-export.png)

*Figure 17: Exporting SAP Cloud Platform SDK for iOS frameworks from the Assistant*

![Figure 18: Repository contents after pods have been uploaded](artifactory-4-pods-uploaded.png)

*Figure 18: Repository contents after pods have been uploaded*
...
Proceed to the next screen and create a new application in SAP Cloud Platform as depicted in Figure 20. All of those values are really just Cloud Platform-internal, but ideally you should match what you selected in the first screen for the sake of simplicity.

On the final screen, which is shown in Figure 21, add a new destination to your project by hitting the "+" button and selecting "Existing destination…". From the list, select ``com.sap.edm.sampleservice``, which is an OData service shipped with Mobile Services for just that purpose.

![Figure 19: General project settings in the Assistant](assistant-2-project.png)

*Figure 19: General project settings in the Assistant*

![Figure 20: Selecting the Sample Service in the Assistant](assistant-3-sampleservice.png)

*Figure 20: Selecting the Sample Service in the Assistant*

![Figure 21: Maintaining server-related configuration in the Assistant](assistant-4-server.png)

*Figure 21: Maintaining server-related configuration in the Assistant*

Once the app has been generated, Xcode will automatically open it. Now select your project file and as shown in Figure 22, adjust the team in your main target to match your setup. For you, it may be your organization.

Now we are just going to add some dummy tests in order for our build job to have something to execute. For that purpose, go to the Tests tab and add a new Unit Test Target as shown in Figure 23. This will also add two empty tests by default, which are good enough for what we are trying to do. At this point, you can right-click the target that should now show in the tests list and run it to see if things work so far.

![Figure 22: Setting the team ID for building in Xcode](xcode-2-teamid.png)

*Figure 22: Setting the team ID for building in Xcode*

![Figure 23: Adding tests to the project](xcode-3-tests.png)

*Figure 23: Adding tests to the project*

#### 4.2 Managing Dependencies with CocoaPods

These setup steps are not yet specific to our tutorial, but some basic settings that should be maintained early on. Now we are going to get our hands dirty by removing the included frameworks and using CocoaPods instead: First, expand the Frameworks folder in your project, select all SAP frameworks, right-click and delete them, as demonstrated in Figure 24. Ideally, you should move them to Trash, since otherwise the files might still be lingering in your source code repository, which would needlessly inflate the repository and possibly confuse other team members. Next, we add a Podfile to our project by running the following command from Terminal in the project root directory:

```
$ pod init
```

![Figure 24: Removing frameworks from the project](xcode-4-delete-frameworks.png)

*Figure 24: Removing frameworks from the project*
...
for CocoaPods to download the dependencies and create a properly configured workspace for you. Now in order to proceed, close Xcode, navigate to your project directory, and open the ``.xcworkspace`` file (shown in Figure 25) that CocoaPods created for you. The reason is that if you proceed without the workspace, you simply won't see the dependencies CocoaPods added. See Figure 26 for an example of what your Xcode project should now look like. Once more, run the unit tests in your project to validate everything still works.

![Figure 25: The Xcode workspace generated by CocoaPods](xcode-5-xcworkspace.png)

*Figure 25: The Xcode workspace generated by CocoaPods*

![Figure 26: Generated workspace opened in Xcode](xcode-6-workspace-opened.png)

*Figure 26: Generated workspace opened in Xcode*
...
Now we need to put our project into a Git repository, so that Jenkins can pick it up from builds. First, you will need to create a new Git repository as shown in Figure 27. As mentioned previously, in this guide we are showing the setup using the example of GitHub, so the exact screens might vary for you. Once done, go to the repository "Integration & services" settings as shown in Figure 28 and add the "Jenkins (GitHub plugin)" service to your repository. Finally, in the next screen that pops up, enter the webhook URL of your Jenkins, which is simply ``http(s)://your.host.name:port/github-webhook/``, as shown in Figure 29. Now your Git repository is able to send notifications to your build server whenever a code change happens.

![Figure 27: Creating a new repository in GitHub](git-1-create.png)

*Figure 27: Creating a new repository in GitHub*

![Figure 28: Adding Jenkins plugin to GitHub repository](git-2-jenkins-service.png)

*Figure 28: Adding Jenkins plugin to GitHub repository*

![Figure 29: Setting the webhook URL for source control notifications](git-3-webhook.png)

*Figure 29: Setting the webhook URL for source control notifications*
...
Then, back in your Jenkins web interface, you should see a Welcome message akin to what is shown in Figure 29. Click "create new jobs" to get started, and select the "Freestyle Project" option shown in Figure 30. In the "Build Triggers" section, select "GitHub hook trigger for GITScm polling", as depicted in Figures 31. Then, configure the Pipeline that Jenkins should fetch from Git, as shown in Figure 32.

![Figure 29: Jenkins welcome screen](jenkins-4-create-job.png)

*Figure 29: Jenkins welcome screen*

![Figure 30: Creating a new Pipeline job in Jenkins](jenkins-5-job-type.png)

*Figure 30: Creating a new Pipeline job in Jenkins*

![Figure 31: Webhook configuration in Jenkins job](jenkins-6-scm-poll.png)

*Figure 31: Webhook configuration in Jenkins job*

![Figure 32: Configuring Pipeline from SCM](jenkins-7-scm-pipeline.png)

*Figure 32: Configuring Pipeline from SCM*
...
In your Jenkins web interface, you should now see a new build pop up under the left-hand menu, as shown in Figure 33. Wait for it to complete, which can take a few minutes. Eventually, you should find that the build succeeded, including tests that have been run and a generated ``.ipa`` file in your project root directory. The easiest way to verify this is taking a look at the logs of the build job, and check for the message displayed in Figure 34. If everything is in order, you can now browse to the indicated location and use Xcode to install your very first CI-built iOS app on your phone.

![Figure 33: Jenkins job triggered by a code change in Git](jenkins-8-build.png)

*Figure 33: Jenkins job triggered by a code change in Git*

![Figure 34: Log of the successful Jenkins build stage](jenkins-9-success.png)

*Figure 34: Log of the successful Jenkins build stage*
