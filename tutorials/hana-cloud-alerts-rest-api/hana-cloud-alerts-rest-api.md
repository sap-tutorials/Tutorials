...
4. Available alert rules can also be accessed with the REST API. Add the following content to `AlertsAndMetrics.http` to the REST Client variables section.
...
If you wish to access alerts and metrics for multiple instances with one API call, the service plan admin-api-access can be used as shown below.  In addition, metrics for data lake Relational instances can be accessed using this service plan.  This service plan is not available for trial instances.
