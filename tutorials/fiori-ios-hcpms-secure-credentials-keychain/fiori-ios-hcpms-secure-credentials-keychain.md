...
1. **Add the keychain sharing capability to the app**
...
2. **Download `SwiftKeychainWrapper`**
...
3. **Add the downloaded files to your project**
...
4. **Add helper functions for secure settings**
...
5. **Remove variable declaration `userId`and `password`**
...
6. **Change function `register` to store credentials in the keyring**
...
7. **Change function `createBasicAuthString` to use credentials stored in the keyring**
...
8. **Remove any existing registrations for the app**
...
9. **Run the app an check the new registration**
...
10. **Save the application connection ID in the keychain**
