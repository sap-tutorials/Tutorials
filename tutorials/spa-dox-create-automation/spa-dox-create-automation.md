...
time: 25
tags: [ tutorial>intermediate, software-product>sap-intelligent-robotic-process-automation]
...
---
In this tutorial, you will automate the process of extracting the **Invoice document** using Document Extraction Template and send the invoice details to the process.
...
4. Choose your extraction schema.
  ...
  - Choose the fields that you would extract the data from Invoice document.
...
1. Extract data template
  - Search for the activity **Extract data** and add the activity **Extract Data(Template)** to the
    automation flow.
    > ### What is going on?
...
2.  You have already created the data type **Invoice** in Step 2. Now, you will create variable of the type **Invoice**.
    ...
    - Click on the Edit button (pencil icon) to map  the parameter  `DocumentNumber` to the extracted invoice data from the activity **Extract Data (template)**.
...
3. Print the invoice data using  the activity **Log message**.
    ...
    Add **Log Message** activity and the `myinvoiceData` to the value.
  > ### What is going on?
...
Select the **End** and pass the variable `myinvoiceData` to the output parameter `InvoiceDetails` which you have created in Step2.
...
Select Test button and enter the `Filepath` where the invoice document is stored locally on your machine.
...
Navigate to the process **Get Invoice Details** and select the automation **Extract Invoice Data**.

Map the input parameter of the automation to the form parameter `FilePath`
...
Your process looks like below once you complete this tutorial.
