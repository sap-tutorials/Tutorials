...
[ACCORDION-BEGIN [Step 1: ](Install and manage the Cx-server)]
...
Next, log into your Linux host and navigate to the Cx-server folder. After that, you are ready to start your SAP Cloud SDK Cx-server instance.

### Start the SAP Cloud SDK Cx-server
...
### Stop the SAP Cloud SDK Cx-server
...
[ACCORDION-BEGIN [Step 3: ](Build your first project)]
...
Next, you need to set up the SCM source of your project´s build job. Look for **Branch Sources** and add one by specifying your GitHub Enterprise endpoint, organization (via field **Owner**), and repository.
...
Please note that your pipeline skipped the deployment stage because you did not configure a deployment target yet. To deploy your application to your SAP Cloud Platform account, you need to add a target to your `pipeline_config.yml` file.
...
Finally, you can now leverage this credentials record by adding the credentials configuration property to the `integrationTests` stage of your `pipeline_config.yml`.
...
We recommend to increase the number executors to at least the number of cores of your machine. This will help your Cx-server to finish build jobs faster, especially when running multiple in parallel. The number of executors can be configured in **Manage Jenkins > Configure System** in the section **Maven Project Configuration**.
