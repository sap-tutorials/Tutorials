...
- Implement SAP Note 3353630 in your system to get the latest updates for the SAP Service and Asset Manager Mobile Services App Create transaction `/MERP/CPMS_APPCREATE`.
...
2. Fill in the **Admin API**, **SCC Location Id**, **Virtual Host** & **Port**. Review the **Auto Determine Parallelization** and **Background Job User** parameters (additional info below). To add the `sap-client` header to the Mobile Destinations please see Step 5 (optional). To use an RFC Destination instead of the **Admin API** please see Step 4 (optional). Then execute the transaction.
    ...
    | **MS Admin API or MW Server GUID**                             | Used to establish a connection between SAP Mobile Services and the SAP Backend                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
    | **OData Service Mobile App**                                   | Used to generate the mobile app's offline configuration sent to SAP Mobile Services **Mobile Offline Access** feature.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
    | **OData Service Technical Name**                               | Used to generate the mobile app's offline configuration sent to SAP Mobile Services **Mobile Offline Access** feature.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
    | **OData Service Group Version**                                | Used to generate the mobile app's offline configuration sent to SAP Mobile Services **Mobile Offline Access** feature.                                                                                                                                                                                                                                                                                                                                                                                                                                          |
    ...
    | **Auto Determine Parallelization** or **Parallelized Gateway** | Only use the **Auto Determine Parallelization** checkbox if your SAP Gateway resides on the system executing the report (i.e., Embedded Deployment). Please use the **Parallelized Gateway** checkbox if your SAP Gateway does not reside on the system executing the report (i.e., Hub Deployment). If you are using HUB Deployment, to determine if parallelization is active, please check your SAP Gateway Batch and Technical Service configurations in transactions `/IWBEP/BATCH_CONF` and `/IWBEP/CONF_SERVICE` in the system your SAP Gateway resides. |
...
3. Execute the program **`/MERP/CORE_OFFLINE_CONFIG_PROG`** in transaction **SE38** from the SAP GUI, then select your desired variant.

4. Review parameter **Auto Determine Parallelization**. Then execute the transaction.

    ![OfflineProg](offlineprog.png)

    >Only use the **Auto Determine Parallelization** checkbox if your SAP Gateway resides on the system executing the report (i.e., Embedded Deployment). Please use the **Parallelized Gateway** checkbox if your SAP Gateway does not reside on the system executing the report (i.e., Hub Deployment). If you are using HUB Deployment, to determine if parallelization is active, please check your SAP Gateway Batch and Technical Service configurations in transactions `/IWBEP/BATCH_CONF` and `/IWBEP/CONF_SERVICE` in the system your SAP Gateway resides.

5. Please ensure to save the generated file with a `.ini` file extension.

6. Import the file in the **Mobile Offline Access** feature of your app.
