...
time: 25
tags: [ tutorial>beginner, topic>sapui5, products>sap-fiori, products>sap-business-technology-platform, products>sap-btp-for-the-cloud-foundry-environment]
...
You'll build an application that presents a list of suppliers from an on-premise backend. The suppliers' data will be retrieved from the `BusinessPartnerSet` collection in the `GWSAMPLE_BASIC` OData service that is available from SAP's ES5 demo gateway.
...
6. For **HTML5 Applications**, select **Approuter Managed by SAP BTP** and provide a unique service name, e.g. **BP**, and click **Next**.
