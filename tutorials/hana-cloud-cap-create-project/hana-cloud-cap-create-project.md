...
4. For one last change we need to adjust the content generated by the wizard for the **db** folder. For this we will use an open source sample utility called hana-cli.  From this same terminal window run the command ```npm install -g hana-cli```
