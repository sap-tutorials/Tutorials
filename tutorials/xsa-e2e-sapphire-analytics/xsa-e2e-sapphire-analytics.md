...
Once the module has been created, **expand it**, right-click on the `src` folder and choose `Import->File or Project`
...
**Delete** the `src (analytics)` from the path and click **OK**
...
Go back into SAP Web IDE for SAP HANA and **double-click** on the file `responses.csv` to open it. Then use **Ctrl+V** to paste the contents of the survey.

![New DB module](16.png)

**Save** the file.

**Right-click** on the `hana` module and choose **Build** from the contextual menu.
...
Right-click on the folder `src` and choose `New -> Calculation View`.

![New TA CV](cv.png)

Click on the `Aggregation` node and then on the **+** sign to add a new data source.

![New TA CV](19png.png)

**Type in** `respon` to find the set of response you loaded. **Choose** the result starting with `$TA` and click **Finish**.

![New TA CV](20.png)

**Expand** the detail panel and double click on the column `TA_TOKEN` to transfer it to the results. **Click** on `Calculated Columns`.
...
Click on the counter to edit the details.
...
Click on the `anonymize` data node and then click on the white modelling area to drop it
...
Use the button to expand details and **double-click** on the data source to transfer the field to the results.

![New view](33.png)

Then click on **Details**. Configure the anonymity algorithm as follows:
...
Click **Back** once you are finished.
...
Click on the aggregation node and **double-click** on the root data source to transfer the columns to the output
