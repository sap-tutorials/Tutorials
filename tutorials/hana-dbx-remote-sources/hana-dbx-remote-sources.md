...
8. Set the **Object Name** to be `VT_TOURIST_REVIEWS` and the **Schema** to be **`HOTEL`**.
    ...
    >
    ```SQL
    CREATE VIRTUAL TABLE VT_TOURIST_REVIEWS AT "REMOTE_HC"."HC_HDB".HOTEL."TOURIST_REVIEWS";
    ```
    >
...
11. Add a new review.
    ...
    ```SQL
    INSERT INTO HOTEL.VT_TOURIST_REVIEWS(review_id, review_date, destination_id, destination_rating, review) VALUES(3, '2020-08-21', 1, 5, 'The harbour cruise was fantastic.  It was great to see the city from a different viewpoint');
    SELECT * FROM HOTEL.VT_TOURIST_REVIEWS;
    ```    
...
6. Right-click on **`HC_DL`** and choose **Open**.  Create a virtual table in SAP HANA Cloud named **`VT_DL_TOURIST_REVIEWS`** in the schema **HOTEL** that maps to the newly created table in SAP HANA Cloud, data lake Relational Engine.
    ...
    >
    ```SQL
    CREATE VIRTUAL TABLE VT_DL_TOURIST_REVIEWS AT HC_DL.iqaas.HOTEL.TOURIST_REVIEWS;
    ```
    >
    ...
    CREATE VIRTUAL TABLE VT_DL_TOURIST_REVIEWS (
      ...
      DESTINATION_RATING INTEGER,
      REVIEW VARCHAR(500) NOT NULL
    ) AT HC_DL.iqaas.HOTEL.TOURIST_REVIEWS WITH REMOTE;
