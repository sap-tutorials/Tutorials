...
## Prerequisites  
- **Tutorials:** [Get a Free Trial Account on SAP Cloud Platform](hcp-create-trial-account) and [Enable SAP Cloud Platform Mobile Services](https://developers.sap.com/tutorials/fiori-ios-hcpms-setup.html)
...
During the time you will spend following this tutorial series, you will create a project, implement your own UI, access a sample backend provided by the SAP Cloud Platform as well as using a Core ML model to classify product images.
...
Please open up the **SAP Cloud Platform SDK for iOS Assistant** and click **Create new**.

![Project creation](fiori-ios-scpms-teched19-04.png)

Next, click **Create New Application** to start the workflow. This workflow will let you create everything from scratch.

![Project creation](fiori-ios-scpms-teched19-04a.png)

In the first step you can see the previously created Mobile Services account. This should be pre-selected and you can just click **Next**.
...
> In case you're using Mobile Services for a longer time, and have a trial version, you have to check that you don't exceed 5 application definitions on Mobile Services. If you already have 5 applications, you have to delete one of them to be able to create this project.
...
The **Features** step allows you to select which features of the SDK you want to use. Selecting the corresponding features will also create the basic configurations on Mobile Services. Please leave everything as is and click **Next**.
...
In the next step you could make additional changes to your assigned destination regarding the proxy class generation. Usually it is sufficient to just leave it as is. Please click **Next**.
...
The SAP Cloud Platform SDK for iOS Assistant will now create the cloud application on Mobile Services, pull the metadata document of the OData service and generate an Xcode project, including the onboarding flow and all the necessary proxy classes and a convenience data service API.
...
The next screen shows an example for a **Data Privacy** screen, which allows you to add data privacy and consent texts to comply to EU General Data Protection Regulation (GDPR). Please tap on **Allow** to continue with the flow, tapping on **Deny** will cause the app to go back to the initial screen of the flow.

![App Onboarding](fiori-ios-scpms-teched19-16.png)

Also an optional screen is the passcode screen. Using a passcode can be used for an additional layer of security for your app, used for example for apps accessing sensitive data like customer data, sales numbers, and the like.
...
Before you tap **Next**, please click on `Hardware -> FaceID -> Enrolled`. This will cause the simulator to automatically use `FaceID` or `TouchID` to unlock the app instead of asking you for a passcode.

After you enroll in `TouchID` or `FaceID` (depending on the simulator you're using), tap **Next**.

![App Onboarding](fiori-ios-scpms-teched19-17.png)

 Please confirm your passcode and tap **Done**.

![App Onboarding](fiori-ios-scpms-teched19-18.png)

 Because you've enrolled in `TouchID` or `FaceID` a screen will be added to the onboarding flow asking you to authorize the app to use Apple's native security feature. Please tap **Enable** to continue.

![App Onboarding](fiori-ios-scpms-teched19-19.png)

 Congratulations, you've successfully onboarded in your app!

 You should see a table view allowing you to navigate the OData service's entities and also create, update or delete those.
