...
Since Postman does not offer a "Token" authorization type, we must change the type back to "No Auth" so our new token doesn't get over-written back to a Basic Auth. Go to the Authorization tab and select **No Auth** from the drop-down.
