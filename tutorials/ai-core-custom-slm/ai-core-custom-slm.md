...
### Pulling nemotron-mini and Performing Inference

Now we need to import nemotron-mini to our ollama pod before we can infrence the model so here we would be using SAP AI API to call pull model call in Ollama.
...
- Once you have deployed the model in SAP AI Core, you can use the pull endpoint to load additional models, such as nemotron-mini.
```json
{
    "name": "nemotron-mini"
}
